<template>
    ...
    <div class="content__label">
        <div class="content__title" @click="() => this.isSelectedAll = !this.isSelectedAll">Nhân viên</div>
    ...
    <div class="content__main">
            ...
            <div class="content__feature--left">
                <div class="sticky" style="z-index: 11">
                    ...
                    <div class="button__child--sticky" v-show="isOpenMultiDelete">
                        <div class="button__child--sticky__item" @click="isDeleteMultiEmployee()">Xóa</div>
                    </div>
                </div>
            </div>
        ...
        <div class="content__table">
            <div class="table" id="tbEmployeeList">
                <table>
                    <thead>
                        <th class="column--center column--25 table__checkbox"><base-checkbox :stateCheckAll="checkedAllTr(this.paging.pageNumber)" :ref="'checkedAll'" @click="selectAllTr()"></base-checkbox></th>
                        ...
                        <th class="column--250" title="Chi nhánh tài khoản ngân hàng">Chi Nhánh TK ngân hàng </th>
                    ...
                    <tbody>
                        ...
                        @click="selectingTr(emp.employeeId)">
                            ...
                            <td class="column--center table__checkbox " >
                                <base-checkbox :ref="emp.employeeId" :stateCheckAll="checkSelectedTr(emp.employeeId)" @click="selectedTr(emp.employeeId),selectedSingleTr(index, emp.employeeId)"></base-checkbox>
                            </td>
                            <td>{{emp.employeeCode}}</td>
                            <td class="nowrap column--250">{{emp.fullName}}</td>
                            <td class="nowrap column--125">{{Common.formatGender(emp.gender)}}</td>
                            <td class="nowrap column--center column--150">{{Common.formatDate(emp.dateOfBirth)}}</td>
                            ...
                            <td class="table__feature column--center column__fix">
                                <div class="dropdown flex--jc-end">
                                    <button class="dropdown__input pd--6-1-6-16 bgc--inherit" @click="openEmployeeDetail(emp)">
        ...
        <div class="content__footer">
            ...
            <div class="footer__right">
                ...
                <div class="page__number">
                    <button class="mgr--8 btnFrontPage" :class="{'button--disable':hidePreNextPage(this.pagePreNext.pre)}" @click="prePage()">Trước</button>
                    ...
                    <button class="mgl--8 btnBehindPage" :class="{'button--disable':hidePreNextPage(this.pagePreNext.next)}" @click="nextPage()">Sau</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        selectedTr(EmployeeId){
            ...
            for (const tr of this.trSelected[index]) { // a[1][array]
                if(EmployeeId == tr){
                    const temp = this.trSelected[index].filter(function(e) { return e !== EmployeeId })
        ...
        selectAllTr(){
            ...
            if(index == this.paging.pageTotal){
                let maxSizeLastPage = this.totalData - (index - 1)*this.paging.pageSize;
        ...
        async deleteSingleEmployee(){
            ...
            .then(function(){
                me.trSelected[me.paging.pageNumber] = me.trSelected[me.paging.pageNumber].filter(item => item!==me.idDeleteEmployee);
                setTimeout(() => {
                    me.pagingData();
                    }, 200
                );
